...
Fields are typed (float/**integer**/**boolean**); values are stored as `f64` (ints exact to 2^53, bools 0/1) with the type remembered per series and reported in query results. Aggregations: mean/sum/min/max/count/first/last plus **`rate`** (per-second change across the bucket, for counters) and **`percentile`** (`agg:"percentile"`+`p`, or shorthand `p95`/`p99`; linear interpolation).

Wire routing (`tsdb_bridge.rs`): `engine: "tsdb"` (or reserved `tsdb` command), `cmd: "tsdb"` with an `op`: `write` (points: measurement/tags/fields/ts — JSON bool→boolean, integer→integer, else float), **`write_lp`** (InfluxDB line protocol in an `lp` string; ms timestamps, `now()` when absent — `line_protocol.rs`), `query` (measurement + field + tag filters + `[start,end)` + optional `GROUP BY time(interval)` downsample + `group_by` tags + `agg`), `stats`, `retention` (cutoff), `checkpoint`. RBAC gates `tsdb` at `ReadWrite`; Read role is query-only. Node-local.
