...
A standalone time-series engine (InfluxDB-style), mounted like SQL. Off by default. **MVP: in-memory** (no disk persistence yet). Storage differentiator: each series (measurement × tag-set × field) is a **Gorilla-compressed** columnar stream (delta-of-delta timestamps + XOR floats; ~0.3 bytes/point on regular data), partitioned into sealed blocks so retention drops whole expired blocks.
- `OXIDB_TSDB` (`1`/`true`/`yes`/`on` to enable; default off — zero cost when unused). Per-database, like SQL.

Wire routing (`tsdb_bridge.rs`): `engine: "tsdb"` (or reserved `tsdb` command), `cmd: "tsdb"` with an `op`: `write` (points: measurement/tags/fields/ts), `query` (measurement + field + tag filters + `[start,end)` + optional `GROUP BY time(interval)` downsample + `group_by` tags + `agg` mean/sum/min/max/count/first/last), `stats`, `retention` (cutoff). RBAC gates `tsdb` at `ReadWrite`; Read role is query-only. Node-local.
