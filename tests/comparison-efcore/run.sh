...
echo -e "${BOLD}╔══════════════════════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║       OxiDB vs PostgreSQL — EF Core Comparison              ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════════════╝${RESET}"
echo ""
...
echo -e "${GREEN}  OxiDB, PostgreSQL, and SQL Server services are up and healthy.${RESET}"
echo ""

# ── Step 2: Restore .NET dependencies ─────────────────────────────────
echo -e "${CYAN}[2/3] Restoring .NET dependencies...${RESET}"
dotnet restore
echo -e "${GREEN}  Dependencies ready.${RESET}"
echo ""

# ── Step 3: Run tests ────────────────────────────────────────────────
echo -e "${CYAN}[3/3] Running EF Core comparison tests...${RESET}"
echo ""
set +e
dotnet test --no-restore -v normal --logger "console;verbosity=detailed" 2>&1 | tee test_output.log
TEST_EXIT=$?
set -e
echo ""
