...
class OxiDbClient:
    ...
    def extract_text(self, bucket: str, key: str) -> str:
        """Extract text from a blob object using OxiDB's built-in extractors (PDF, DOCX, HTML, etc.)."""
        result = self._checked({"cmd": "extract_text", "bucket": bucket, "key": key})
        return result["text"]
    ...
    def use_database(self, name: str):
        ...
        return self._checked({"cmd": "use_db", "name": name})

    def create_database(self, name: str):
        """Create a new database."""
        return self._checked({"cmd": "create_database", "name": name})

    def drop_database(self, name: str):
        """Drop a database. Cannot drop the default 'oxidb' database."""
        return self._checked({"cmd": "drop_database", "name": name})
    ...
    # ------------------------------------------------------------------
    # Per-database user permissions
    # ------------------------------------------------------------------

    def grant_db_role(self, username: str, database: str, role: str):
        """Grant a per-database role to a user. Role can be 'admin', 'readWrite', or 'read'.
    ...
    def revoke_db_role(self, username: str, database: str):
        """Revoke a per-database role override from a user, reverting to their global role.
        Requires Admin privileges."""
        return self._checked({
