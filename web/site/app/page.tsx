...
export default function Page() {
  ...
  <div class="container">
    <p class="hero-kicker"><span class="hero-pulse"></span>v0.42.12 &middot; realtime subscriptions that filter &middot; polygon geofences + <code>$geoNear</code> &middot; MCP server for AI assistants &middot; <a href="/license/">source-available</a>, free for production use</p>
    ...
    <p class="sub">One database process, several shapes of data: <strong>JSON documents</strong> with MongoDB-style queries, aggregation pipelines, geospatial and graph stages &middot; a <strong>full SQL engine</strong> with joins, window functions, stored procedures and an EF Core provider &mdash; reachable over the <strong>PostgreSQL wire protocol</strong>, so psql, JDBC, Npgsql and DBeaver connect unmodified &middot; Gorilla-compressed <strong>time series</strong> with continuous rollups &middot; a <strong>Redis-compatible</strong> key-value store with MULTI/EXEC and Lua &middot; <strong>S3-compatible</strong> object storage that aws-cli and boto3 speak to unmodified &middot; <strong>MQTT/AMQP</strong> messaging, bridged across protocols &middot; with <strong>full-text &amp; vector search</strong> over all of it.</p>
    <p class="sub">ACID transactions, snapshot-consistent reads, Raft replication, point-in-time recovery, SCRAM/TLS/RBAC and AES-GCM encryption at rest are built in &mdash; one ~20 MB zero-dependency binary that starts with no configuration, runs embedded in your process, and compiles to WASM for the browser.</p>
...
curl -LO https://oxidb.baltavista.com/releases/v0.42.12/oxidb-server-v0.42.12-darwin-arm64.tar.gz
tar xzf oxidb-server-v0.42.12-darwin-arm64.tar.gz

<span class="co"># Windows: download oxidb-server-v0.42.12-windows-amd64.zip from /downloads/</span></code></pre>
