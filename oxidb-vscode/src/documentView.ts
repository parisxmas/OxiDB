...
import { OxiDBClient, SqlResult } from './client';
...
function sqlResultHtml(r: SqlResult, i: number, total: number): string {
...
export class SqlResultsView {
  private panel: vscode.WebviewPanel | null = null;

  show(sql: string, results: SqlResult[], elapsedMs: number): void {
    if (!this.panel) {
...
export class DocumentViewProvider {
  ...
  constructor(private client: OxiDBClient) {}

  async showCollection(collection: string): Promise<void> {
  ...
  async showQueryResults(title: string, data: any): Promise<void> {
  ...
  private getHtml(collection: string, docs: any[], count: number, indexes: any[]): string {
...
<div class="stats">${count} documents | ${indexes.length} indexes: ${indexes.map((i: any) => `<span class="idx">${esc(i.name || i.field)}</span>`).join(' ')}</div>
...
<tbody>${docs.map((d: any) => `<tr>${cols.map((c) => `<td title="${esc(JSON.stringify(d[c]))}">${esc(fmt(d[c]))}</td>`).join('')}</tr>`).join('')}</tbody></table>
</div>
...
window.addEventListener('message', e => {
  ...
  private getResultsHtml(title: string, docs: any[]): string {
    return `<!DOCTYPE html>
<html><head>
<style>${BASE_STYLE}</style>
</head><body>
