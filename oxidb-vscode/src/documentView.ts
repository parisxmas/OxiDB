...
<h2>${collection}</h2>
<div class="stats">${count} documents | ${indexes.length} indexes: ${indexes.map((i: any) => `<span class="idx">${i.name || i.field}</span>`).join(' ')}</div>
...
<tbody>${docs.map((d: any) => `<tr>${cols.map((c) => `<td title="${this.esc(JSON.stringify(d[c]))}">${this.esc(this.fmt(d[c]))}</td>`).join('')}</tr>`).join('')}</tbody></table>
</div>
