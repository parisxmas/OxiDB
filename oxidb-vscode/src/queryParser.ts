...
export function parseShellQuery(input: string): any | null {
  ...
  const match = trimmed.match(
    /^(?:db\.)?(\w+)\.(find|findOne|find_one|insert|insertMany|insert_many|update|updateOne|update_one|delete|deleteOne|delete_one|deleteMany|count|aggregate|createIndex|create_index|drop|listIndexes|list_indexes)\s*\(([\s\S]*)\)$/
  );
  ...
  switch (method) {
    case 'find': {
      const cmd: any = { cmd: 'find', collection, query: parseJsonArg(args[0]) || {} };
      if (args[1]) {
      ...
      if (args[2]) {
        ...
        if (!isNaN(limit)) { cmd.limit = limit; }
      }
      if (!cmd.limit) { cmd.limit = 50; }
      return cmd;
    }
...
function parseJsonArg(arg: string | undefined): any {
