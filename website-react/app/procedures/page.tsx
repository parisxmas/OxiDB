...
COMMIT;                       <span class="co">-- all-or-nothing</span></code></pre>
    ...
    <p class="section-desc"><strong>Cobra</strong> is the compiled procedure language. You author a small program, compile it to portable <code>.cobrac</code> bytecode, and the server executes it on a built-in Rust VM &mdash; no toolchain on the server, no <code>cgo</code>, no sidecar process. The program defines <code>run(db, &hellip;params)</code>; the <code>db</code> handle goes through the same executor as ordinary SQL.</p>
