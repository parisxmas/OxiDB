...
SELECT NEXT VALUE FOR order_seq;               <span class="co">-- EF Core HiLo keys</span></code></pre>
    ...
    <p>Two languages: zero-toolchain <strong>SQL-text</strong> bodies and compiled <strong>Cobra</strong> bytecode run by an in-server VM. Both <code>CALL</code> inside the caller's transaction and replicate under Raft.</p>
...
CALL give_raise(5);</code></pre>
    <p>Full guide &mdash; SQL-text vs Cobra, the <code>run(db, &hellip;)</code> API, compiling, result shaping, and replication safety: <a href="/procedures/">Stored Procedures</a>.</p>
