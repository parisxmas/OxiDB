...
fn setup() -> (tempfile::TempDir, oxidb_sql::SqlEngine) {
    let (dir, db) = open();
    db.execute("CREATE TABLE t (id INT, a INT, s TEXT)")
        .unwrap();
    for (id, a) in [(1, "1"), (2, "5"), (3, "9"), (4, "NULL")] {
        db.execute(&format!("INSERT INTO t (id, a) VALUES ({id}, {a})"))
            .unwrap();
    }
    (dir, db)
}
...
fn between_is_inclusive_at_both_ends() {
    let (_d, db) = setup();
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE a BETWEEN 1 AND 9"),
        vec![1, 2, 3]
    );
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE a BETWEEN 5 AND 5"),
        vec![2]
    );
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE a BETWEEN 2 AND 8"),
        vec![2]
    );
}
...
fn an_empty_range_matches_nothing() {
    ...
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE a BETWEEN 8 AND 2"),
        Vec::<i64>::new()
    );
}
...
fn not_between_excludes_the_range() {
    let (_d, db) = setup();
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE a NOT BETWEEN 2 AND 8"),
        vec![1, 3]
    );
}
...
fn a_null_operand_is_null_in_both_directions() {
    ...
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE (a BETWEEN 1 AND 9) IS NULL"),
        vec![4]
    );
    assert_eq!(
        ids(
            &db,
            "SELECT id FROM t WHERE (a NOT BETWEEN 1 AND 9) IS NULL"
        ),
        vec![4]
    );
}
...
fn between_partitions_the_table_ternary() {
    ...
    assert_eq!(
        all,
        vec![1, 2, 3, 4],
        "every row belongs to exactly one partition"
    );
}
...
fn between_agrees_with_the_comparison_it_desugars_to() {
    ...
    for (lo, hi) in [(1, 9), (2, 8), (5, 5), (0, 100), (8, 2)] {
        assert_eq!(
            ids(
                &db,
                &format!("SELECT id FROM t WHERE a BETWEEN {lo} AND {hi}")
            ),
            ids(
                &db,
                &format!("SELECT id FROM t WHERE a >= {lo} AND a <= {hi}")
            ),
        ...
        assert_eq!(
            ids(
                &db,
                &format!("SELECT id FROM t WHERE a NOT BETWEEN {lo} AND {hi}")
            ),
...
fn between_works_on_text_and_on_expressions() {
    let (_d, db) = setup();
    db.execute("INSERT INTO t (id, s) VALUES (10, 'bbb')")
        .unwrap();
    db.execute("INSERT INTO t (id, s) VALUES (11, 'zzz')")
        .unwrap();
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE s BETWEEN 'a' AND 'c'"),
        vec![10]
    );
    // The operand does not have to be a bare column.
    assert_eq!(
        ids(&db, "SELECT id FROM t WHERE a + 1 BETWEEN 2 AND 6"),
        vec![1, 2]
    );
}
