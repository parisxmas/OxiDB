...
OxiDB provides official client libraries for Python, Go, Julia, .NET, and Swift. All clients communicate with the server using the length-prefixed JSON [protocol](protocol-reference.md) over TCP.
...
# Custom host/port
client = OxiDbClient(host="192.0.2.1", port=5555, timeout=10.0)
...
// Custom
client, err := oxidb.Connect("192.0.2.1", 5555, 10*time.Second)
```
