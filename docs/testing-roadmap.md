...
| 5 | **Security** | ✅ partial — wire-protocol fuzz harness in [`fuzz/`](../fuzz/) (4 mutation + 3 structure-aware roundtrip + 2 cross-impl differential) + [OSS-Fuzz integration infra](../infra/oss-fuzz/) (upstream PR pending) + [authn/authz bypass corpus](../oxidb-server/tests/security_authn_authz.rs) (12 named attack patterns, all rejected). 7 DoS + 1 correctness bug found+fixed via the harness | OSS-Fuzz upstream submission to `google/oss-fuzz`; external pentest (Cure53 / Trail of Bits); SCRAM-SHA-256 stateful fuzz — see [ADR-0006 §5](decisions/0006-cern-testing-gap-estimates.md#category-5--security) for effort estimates |
