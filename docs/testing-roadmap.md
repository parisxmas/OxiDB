...
| 5 | **Security** | ✅ partial — wire-protocol fuzz harness in [`fuzz/`](../fuzz/) — 4 mutation targets + 3 structure-aware roundtrips + 2 cross-impl differential targets (RESP vs `redis-rs`, pg_wire vs `pgwire` crate) + [OSS-Fuzz integration infra](../infra/oss-fuzz/) (upstream PR pending). 7 DoS bugs + 1 correctness bug found+fixed via the harness | OSS-Fuzz upstream submission to `google/oss-fuzz`; external pentest (Cure53 / Trail of Bits); authn/authz bypass tests — see [ADR-0006 §5](decisions/0006-cern-testing-gap-estimates.md#category-5--security) for effort estimates |
