...
| 5 | **Security** | ✅ partial — wire-protocol fuzz harness in [`fuzz/`](../fuzz/) — 4 mutation targets (dispatcher, OxiWire, RESP, pg_wire) + 3 structure-aware targets (OxiWire / RESP / MsgPack roundtrip, MsgPack cross-impl-diff vs `rmp_serde`) + [OSS-Fuzz integration infra in `infra/oss-fuzz/`](../infra/oss-fuzz/) (upstream PR pending) | Differential fuzz vs real Redis / Postgres; OSS-Fuzz upstream submission to `google/oss-fuzz`; external pentest (Cure53 / Trail of Bits); authn/authz bypass tests — see [ADR-0006 §5](decisions/0006-cern-testing-gap-estimates.md#category-5--security) for effort estimates |
