...
| 4 | **HA / Raft fault injection** | ✅ partial — 6 tests in [`oxidb-server/tests/raft_test.rs`](../oxidb-server/tests/raft_test.rs) (cluster formation + leader election, write replication, leader-kill failover, data consistency after failover, 5-node-minus-2-followers, minority-can't-elect) | Linearizability checker (Knossos / Elle), network-partition nemesis (vs node kill), randomised chaos ordering, clock skew, slow/asymmetric network, slow disk — each effort-estimated in [ADR-0005](decisions/0005-raft-fault-injection-scope.md) |
| 5 | **Security** | ✅ partial — wire-protocol fuzz harness in [`fuzz/`](../fuzz/) (4 mutation + 3 structure-aware roundtrip + 2 cross-impl differential) + [OSS-Fuzz integration infra](../infra/oss-fuzz/) (upstream PR pending) + authn/authz bypass corpus ([JWT/RBAC](../oxidb-server/tests/security_authn_authz.rs) + [SCRAM stateful](../oxidb-server/tests/security_scram_stateful.rs) + [handler canonicalisation](../oxidb-server/tests/security_handler_canonicalisation.rs) + [audit-log evidence](../oxidb-server/tests/security_audit_evidence.rs) + audit-log rotation [size](../oxidb-server/tests/security_audit_rotation.rs) + [time](../oxidb-server/tests/security_audit_time_rotation.rs) + [calendar-aligned UTC](../oxidb-server/tests/security_audit_calendar_rotation.rs) + [env-var policy parser](../oxidb-server/tests/security_audit_env_policy.rs) + [gzip compression](../oxidb-server/tests/security_audit_gzip.rs), 93 attack/property patterns total, all rejected). 7 DoS + 1 correctness bug found+fixed via the harness | OSS-Fuzz upstream submission to `google/oss-fuzz`; external pentest (Cure53 / Trail of Bits) — see [ADR-0006 §5](decisions/0006-cern-testing-gap-estimates.md#category-5--security) for effort estimates |
