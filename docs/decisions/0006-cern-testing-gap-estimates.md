...
| Authn/authz bypass test corpus | ✅ partial — [`oxidb-server/tests/security_authn_authz.rs`](../../oxidb-server/tests/security_authn_authz.rs) covers 8 JWT attack patterns (role-escalation payload tamper, wrong secret, expired, alg=none header bypass, garbage signature, missing exp, malformed structure, empty-secret universal-unlock) + 4 RBAC fail-closed pins (unknown commands, uppercase case-mismatch, ReadWrite-can't-admin, Read locked down). 12/12 pass. SCRAM-SHA-256 fuzz still pending — needs a stateful harness for the multi-round protocol. |
