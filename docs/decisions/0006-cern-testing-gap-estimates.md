...
| Authn/authz bypass test corpus | ✅ landed — JWT + RBAC ([`security_authn_authz.rs`](../../oxidb-server/tests/security_authn_authz.rs), 12 patterns) + SCRAM stateful ([`security_scram_stateful.rs`](../../oxidb-server/tests/security_scram_stateful.rs), 11 patterns) + handler-level canonicalisation ([`security_handler_canonicalisation.rs`](../../oxidb-server/tests/security_handler_canonicalisation.rs), 9 patterns) + per-vector audit-log evidence shape ([`security_audit_evidence.rs`](../../oxidb-server/tests/security_audit_evidence.rs), 8 patterns) + audit-log size-based rotation ([`security_audit_rotation.rs`](../../oxidb-server/tests/security_audit_rotation.rs), 5 patterns) + audit-log **time-based** rotation ([`security_audit_time_rotation.rs`](../../oxidb-server/tests/security_audit_time_rotation.rs), 8 patterns) + audit-log **calendar-aligned** rotation ([`security_audit_calendar_rotation.rs`](../../oxidb-server/tests/security_audit_calendar_rotation.rs), 13 patterns) + audit-log **env-var policy parser** ([`security_audit_env_policy.rs`](../../oxidb-server/tests/security_audit_env_policy.rs), 15 patterns) + audit-log **gzip compression of rotated files** ([`security_audit_gzip.rs`](../../oxidb-server/tests/security_audit_gzip.rs), 12 patterns covering off-by-default contract, .gz produced on rotation, plaintext deleted on success, decompressed content matches exactly, env var parsing for `OXIDB_AUDIT_COMPRESS` (all bool aliases), composition with other triggers). 93 total tests, all currently pass. Rotation primitive now exposes `RotationPolicy { max_bytes, max_age, calendar, compress }` wired to env vars in `main.rs` (both standalone + cluster modes). |
