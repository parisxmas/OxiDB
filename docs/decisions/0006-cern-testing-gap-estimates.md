...
| Authn/authz bypass test corpus | ✅ landed — JWT + RBAC corpus in [`oxidb-server/tests/security_authn_authz.rs`](../../oxidb-server/tests/security_authn_authz.rs) (12 attack patterns) + SCRAM-SHA-256 stateful corpus in [`oxidb-server/tests/security_scram_stateful.rs`](../../oxidb-server/tests/security_scram_stateful.rs) (10 attacks: malformed GS2 header, unknown user, empty username, control-char injection, wrong password, nonce mismatch, malformed proof, zero proof, missing fields, cross-session replay; + happy-path baseline). 23 total tests, all currently pass. |
