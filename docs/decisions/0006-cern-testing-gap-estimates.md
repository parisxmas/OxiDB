...
| Authn/authz bypass test corpus | ✅ landed — JWT + RBAC ([`security_authn_authz.rs`](../../oxidb-server/tests/security_authn_authz.rs), 12 patterns) + SCRAM stateful ([`security_scram_stateful.rs`](../../oxidb-server/tests/security_scram_stateful.rs), 11 patterns) + handler-level command canonicalisation ([`security_handler_canonicalisation.rs`](../../oxidb-server/tests/security_handler_canonicalisation.rs), 9 patterns covering case-sensitivity, non-string `cmd` types, whitespace padding, pathological inputs). 32 total tests, all currently pass. |
