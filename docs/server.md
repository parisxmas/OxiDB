...
| `OXIDB_SQL_DISK_FIRST` | **on** | SQL disk-first row storage: table data served from the mmap'd last-checkpoint snapshot, only post-checkpoint changes in RAM. `0` restores all-rows-resident mode |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `OXIDB_GELF_ADDR` | - | GELF UDP endpoint for remote logging (e.g., `192.0.2.100:12201`) |
...
```bash
OXIDB_GELF_ADDR=192.0.2.100:12201 ./oxidb-server
```
