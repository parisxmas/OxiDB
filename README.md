...
**Adoption status:** pre-1.0 — not production-ready, not ready for **[CERN / enterprise / large-scale research deployment](docs/cern-compatibility.md)**. Path to 1.0 stability is in [ADR-0003](docs/decisions/0003-1.0-stability-scope.md); release policy in [ADR-0004](docs/decisions/0004-phase-0-answers.md). All architectural decisions: [`docs/decisions/`](docs/decisions/).
...
> 🔬 **Quality & testing.** All 8 categories of the [CERN-grade testing roadmap](docs/testing-roadmap.md) are at ✅ partial: ACID isolation pinned across 4 anomalies (sits between READ COMMITTED and SI per OCC's read-set commit-time validation), crash recovery covers soft + hard SIGKILL + byte-offset matrix, performance soak with WAL-bound check, scale workload at 100K docs, Raft (7 cluster-fault tests), DR drill (backup→wipe→restore, ~30ms RTO; encrypted variant verifies no plaintext leak on wrong key), upgrade-chain fixture corpus, **9-target fuzz harness** (4 mutation + 3 structure-aware roundtrip + 2 cross-impl differential vs `redis-rs` and the `pgwire` crate) with [coverage reporting](fuzz/coverage.sh) showing structure-aware reaches up to 55% line coverage. Fuzz harness has found **7 DoS bugs** + 1 correctness bug in TCP-facing decoders, all fixed across PRs #46–#65. Remaining gaps have effort estimates in [ADR-0006](docs/decisions/0006-cern-testing-gap-estimates.md); OSS-Fuzz integration infra committed in [`infra/oss-fuzz/`](infra/oss-fuzz/) (upstream PR pending).
