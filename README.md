...
## 🔬 CERN-grade testing program

OxiDB is engineered to a **[CERN-grade testing standard](docs/cern-compatibility.md)** — every one of the 8 categories of database hardening has landed first-slice coverage with named tests in-tree, not aspirational tags on a roadmap. The numbers below come from `cargo test`, not marketing.

| # | Category | What's landed |
|---|---|---|
| 1 | **ACID & isolation** | 4 anomalies pinned in [`tests/cern_acid_isolation.rs`](tests/cern_acid_isolation.rs). OxiDB sits **between READ COMMITTED and SNAPSHOT ISOLATION** — OCC validates the read-set at commit even for read-only txs (see [`docs/format/tx-commit-log.md`](docs/format/tx-commit-log.md#isolation-level-observed)). |
| 2 | **Crash recovery** | Soft-crash ([`cern_crash_recovery.rs`](tests/cern_crash_recovery.rs)) + hard SIGKILL ([`cern_sigkill_drill.rs`](tests/cern_sigkill_drill.rs)) + byte-offset SIGKILL matrix ([`cern_sigkill_byte_offset.rs`](tests/cern_sigkill_byte_offset.rs)) — every ACKed write recovers; reopen succeeds at every kill offset including mid-init. |
| 3 | **Performance & long-tail** | Bounded soak with WAL-size growth check ([`cern_soak.rs`](tests/cern_soak.rs)) — 1.73 MiB after 22K mixed ops over 30s; rotation healthy. |
| 4 | **HA / Raft fault injection** | **6 cluster-fault tests** ([`oxidb-server/tests/raft_test.rs`](oxidb-server/tests/raft_test.rs), `cluster` feature): cluster formation + leader election, write replication, leader-kill failover, data consistency after failover, 5-node-minus-2-followers, minority-can't-elect. Gap inventory in [ADR-0005](docs/decisions/0005-raft-fault-injection-scope.md). |
| 5 | **Security** | **9 fuzz targets** in [`fuzz/`](fuzz/) — 4 mutation + 3 structure-aware roundtrip + 2 cross-impl differential (vs canonical `redis-rs` and the `pgwire` crate). [Coverage reporting](fuzz/coverage.sh) shows structure-aware reaches up to **55% line coverage**. **53 named authn/authz attack/property patterns** across 7 test files. **7 DoS bugs + 1 correctness bug found + fixed.** [OSS-Fuzz infra committed](infra/oss-fuzz/) (upstream PR pending). |
| 6 | **Upgrade / migration** | Per-version fixture corpus pattern in [`tests/fixtures/upgrade/`](tests/fixtures/upgrade/) — first fixture `v0.28.4.tar.gz`, reader in [`cern_upgrade_chain.rs`](tests/cern_upgrade_chain.rs). Every shipped version drops a binary fixture; any backward-incompat fails loudly. |
| 7 | **Scale** | HEP-shaped workload at 100K docs in [`cern_scale.rs`](tests/cern_scale.rs): 56k inserts/sec (tx-batched), 30× index speedup over scan, 80k deletes/sec. `OXIDB_SCALE_DOCS=1000000` to crank up. |
| 8 | **Disaster recovery** | Backup → wipe → restore drill in [`cern_dr_drill.rs`](tests/cern_dr_drill.rs) — **~30 ms RTO** for 1500 docs + indexes + blobs. Encrypted variant verifies **no plaintext leak on wrong key** across all 3 sub-cases. |

Every remaining gap has an explicit effort estimate in **[ADR-0006](docs/decisions/0006-cern-testing-gap-estimates.md)** (the "every gap is explicit and effort-estimated" doc). 1.0 stability surface scoped in **[ADR-0003](docs/decisions/0003-1.0-stability-scope.md)**. Full breakdown: **[`docs/testing-roadmap.md`](docs/testing-roadmap.md)**.

---

> ⚠️ **WARNING — pre-1.0 stability.** Despite the testing rigour above, OxiDB is still under active pre-1.0 development. The on-disk data format, the wire/server protocol, the client SDK surface, and the JSON query language are all subject to **breaking changes between releases** with no migration path or backward-compatibility guarantee. Pin a specific version, expect to dump-and-reload on upgrade, and treat any production-like use as experimental until a `1.0` release explicitly commits to stability. Quality ≠ stability — the testing program protects against regressions *within* a version, not breaking changes *across* versions.
