...
| 4 | **HA / Raft fault injection** | 7 cluster-fault tests ([`oxidb-server/tests/raft_test.rs`](oxidb-server/tests/raft_test.rs), `cluster` feature): leader-kill failover, data consistency after failover, 5-node minus 2 followers, minority-can't-elect, split-brain prevention. Gap inventory in [ADR-0005](docs/decisions/0005-raft-fault-injection-scope.md). |
